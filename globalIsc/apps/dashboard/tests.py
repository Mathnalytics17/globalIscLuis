...
class NotificationTopicContractTests(TestCase):
    ...
    def test_internal_global_oil_group_excludes_client_users(self):
        ...
        self.assertEqual(
            topic.resolved_recipient_emails(),
            [internal.email],
        )

    @patch("apps.dashboard.api.views.index.send_mail")
    def test_test_email_endpoint_uses_resolved_recipients(
        self,
        send_mail_mock,
    ):
        user = User.objects.create_user(
        ...
        response = client.post(
            reverse(
                "notification-topics-test-email",
                args=[topic.pk],
            )
        )
        ...
        self.assertEqual(
            ...
            ["laboratorio@example.com"],
        )

        send_mail_mock.assert_called_once()

        self.assertTrue(
            NotificationDispatchLog.objects.filter(
                payload__test=True,
                status="sent",
            ).exists()
        )
    ...
    def test_event_code_is_immutable_after_creation(self):
        ...
        serializer = NotificationTopicSerializer(
            topic,
            data={
                "code": NotificationTopic.EventCode.OVERDUE
            },
            partial=True,
        )
    ...
    def test_delete_deactivates_topic_without_removing_it(self):
        ...
        self.assertTrue(
            NotificationTopic.objects.filter(
                pk=topic.pk
            ).exists()
        )
    ...
    def test_operational_email_is_queued_without_contacting_smtp(self):
        ...
        with patch(
            "apps.dashboard.api.services.operational_center.send_mail"
        ) as send_mail_mock:
    ...
    def test_pending_notification_can_be_processed(
        self,
        send_mail_mock,
    ):
        topic = NotificationTopic.objects.create(
